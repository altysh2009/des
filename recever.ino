...
void bittoascii()
{
 ...
 text="";
  for(int i=0;i<8;i++)
 {
     b=(INbox[0+pos] <<7) +(INbox[1+pos] <<6) +(INbox[2+pos] <<5) +(INbox[3+pos] <<4) +(INbox[4+pos] <<3) + (INbox[5+pos] << 2) + (INbox[6+pos] << 1) + INbox[7+pos];
     text+=char(b);
     //Serial.println(b);
...
//  Serial.print("hallo");
 char sdf=char(BTserial.read());
 if(sdf!='x')
 {
  text+=sdf;
 while(text.length()<16){
  
  uint8_t sdfg=BTserial.read();
  //Serial.println(sdfg);
 if (sdfg<255)
 {text+= char(sdfg);
 }
